...
WorldView::~WorldView()
{
	...
	if (m_haveStars = false) delete m_bgStarShader;
}
...
void WorldView::DrawBgStars() //This is the background starfield
{
	...
	unsigned long seed = UNIVERSE_SEED;
	...
	for (int i=0; i<BG_STAR_MAX; i++) {
		float col = (float)rand.Double(0,1); //Pi::rng.NDouble(4); 
		...
		/*if (Pi::player) {
			if (Pi::player->GetFrame()->m_astroBody) {
				star_intensity = altitude/120000.0f;
				col *= Clamp(star_intensity, 0.0f, 1.0f);
				printf("%f :: ",star_intensity);
			}
		}*/
		if (i<6) {
			col = 1.0;
